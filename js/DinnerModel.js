class DinnerModel{
    ...
    searchDishes(dishType, freeText){
      return this.connectAPI("recipes/search?query="+freeText+"&type="+dishType)
    ...
    checkIngredient(dish){
        for(var i=0; i<dish.extendedIngredients.length;i++){
            if(this.ingredients.some(el=>el.id === dish.extendedIngredients[i].id)){
                let obj = this.ingredients.find(o => o.id === dish.extendedIngredients[i].id);
                let index = this.ingredients.indexOf(obj);
                this.ingredients[index].amount+=dish.extendedIngredients[i].amount;
                console.log("amount of" +this.ingredients[index].name+": "+this.ingredients[index].amount);
            }else{
    ...
    getDishDetails(id){
          return this.connectAPI("recipes/"+id+"/information")
          .catch(console.error);
      }
    ...
    getTotalPrice(dishes){
          if(dishes.length!==0){ //The reduce() method executes a reducer function (that you provide) on each element of the array, resulting in a single output value.
            let reducer=(accumulator, currentValue)=>accumulator+currentValue.dishPrice; //accumulator stores value in each iteration
    ...
    getTotalPriceSum(ingredients){    // Calculate total price to display in Summary view
